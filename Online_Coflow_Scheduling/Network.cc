...
//----------------------------------------------------------------//
// Public Method: write_cstr_released                                 //
//----------------------------------------------------------------//

 void Network::write_cstr_released(GRBModel & model, GRBVar *z)
 {
    GRBLinExpr               lhsExpr1,lhsExpr2;
    int                      t, k, l, id_link;
    double                   F1=0;
    double                   F2=0;
    double                   F = 0;
    ...
    for (l=0; l<nbLinks_; l++) {
      ...
      for (int k = 0; k< nbCoflows_; k++) {
        double start = 0;
        double rk = coflow_[k].getStartTime();
        // cerr << "rk= " <<rk << endl;
        double p_lk = total_transmission_time( id_link, k, true) ;
        lhsExpr1 = z[k] - (rk + p_lk);

        model.addConstr(lhsExpr1, GRB_GREATER_EQUAL, 0.0,  "Start time"+itos(id_link)+"_C_"+itos(k));
      }
      ...
      for (int j = 0; j < Subsets.size(); j++) {
        ...
        for (int k = 0; k< Subsets[j].size(); k++) {
          F1+= pow(total_transmission_time( id_link, Subsets[j][k], false),2);
          F2+=total_transmission_time( id_link, Subsets[j][k], false) ;
          lhsExpr2 +=z[Subsets[j][k]]*total_transmission_time(id_link, Subsets[j][k],false);
        } 
        F = 0.5*(F1+pow(F2,2));
        if ( F > 0 )
          model.addConstr(lhsExpr2, GRB_GREATER_EQUAL, F,  "Par_Ineq_l_"+itos(id_link)+"_S_"+itos(j));
      }
        ...
        for (int k = 0; k < nbCoflows_; ++k) {
            ...
            for (int j = 0; j < nf; ++j) {
              int path_len = coflow_[k].getLength(j);
                ...
                for (int m = 0; m < path_len; ++m) {          
                  if (l == coflow_[k].getLink(j,m)) { //vérifier si le flow utilise le lien l
                    if (RealFlowSizes)
                      sum += coflow_[k].getFlowSize(j);  // p_{l,k}
                    else 
                      sum += coflow_[k].getFlowPredSize(j);  // p_hat_{l,k}
                  }
            }
            ...
            // cerr <<"mu_max= "<< mu_max<<", D= "<< D<<", int of mu_max* D = " << prod <<endl;
            // model.addConstr(load <= prod, "cap_link_l" + itos(l)); //  mu_max* D
            model.addConstr(load, GRB_LESS_EQUAL, prod, "cap_link_l" + itos(l));
          }
    }
}
...
list<int> Network::MUWPSolve_CLP(int alpha, double mu_max, int D, bool RealFlowSizes) {
   list<int> selected_coflows;
    ...
    for (int k = 0; k < nbCoflows_; ++k) {
        for (int l = 0; l < nbLinks_; ++l) {
            double plk = compute_p_lk(coflow_[k], l, RealFlowSizes);
            if (plk > 0) {                
                indices.push_back(l);
    ...
    vector<double> rowUpper(numRows, mu_max * D); // sum p_i,j <= mu_max* D
