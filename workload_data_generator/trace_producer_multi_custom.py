...
## added by Bala
NUM_SAMPLES = 1
NUM_SAMPLES = int(sys.argv[8]); #number of sample output files to be generated without having to reread the trace
# SEED = int(sys.argv[11].zfill(4) + sys.argv[10].zfill(3) + sys.argv[1].zfill(3))
# np.random.seed(SEED)
#print(SEED)

## Added by Aminata
CoflowRelasedTimeMethod= (sys.argv[11]);
...
if(ALPHA=='FB-UP'):
    ...
    DESTINATION_DATA_DIST = sys.argv[6] #'U' for uniform, 'Z' for zipf, 'N' for normal
    NUM_FILE = sys.argv[7]
    FILE = str(NUM_COFLOWS)+'-'+str(LOAD_FACTOR)+'-'+str(ALPHA)+'-'+str(INTRA_COFLOW_CONTENTION*100)+'-'+str(SOURCE_NUM_DIST)+'-'+str(DESTINATION_DATA_DIST)+'-'+str(NUM_FILE)
...
for i in range(NUM_COFLOWS):
    ...
    if(ALPHA=='FB-UP'):
        ...
        if(SOURCE_NUM_DIST=='U'):
            ...
            MAX_NUM_SOURCES = min(ALPHA,NUM_INP_PORTS);
            num_sources = min(max(1,int(np.squeeze(np.random.zipf(2)))),MAX_NUM_SOURCES);
        elif(SOURCE_NUM_DIST=='FB'):
            MAX_NUM_SOURCES = min(ALPHA,NUM_INP_PORTS);
            # MAX_NUM_SOURCES = 150; # real code
            num_sources = min(MAX_NUM_SOURCES,max(1,int(np.squeeze(hist_dist_num_sources.rvs(size=1)))));
        ...
        # print(f"ir: ${ir}")
        num_destinations = min(max(int(np.squeeze(num_sources*ir)),1),NUM_INP_PORTS);
        # print(f"*****num_destinations: ${num_destinations}")
        ...
        for s in list(sources):
            ...
            num_destinations_s = min(max(1,int(np.squeeze((1 + (num_sources - 1)*INTRA_COFLOW_CONTENTION)*ir))),NUM_INP_PORTS);
            # print(num_sources);
            # print(ir);
            # print(num_destinations);
        ...
        for d in list(destinations_set):
            ...
            elif(DESTINATION_DATA_DIST=='N'):
                #proc_time = np.random.normal(MEAN, STDEV);
                #total_data_destination = max(proc_time,0.011);
                #data_across_sources_uniform = np.random.uniform(0,1,num_sources_d);
                #data_across_sources = data_across_sources_uniform * total_data_destination / float(sum(data_across_sources_uniform));
                data_across_sources = np.random.normal(MEAN, STDEV, num_sources_d);
            for l in range(num_sources_d):
                if data_across_sources[l] <= 0.0 :
                    data_across_sources[l] = 0.01;
                flow_size = data_across_sources[l];
                num_flows = num_flows + 1;
                total_flow_size = total_flow_size + flow_size;
                F = {'Coflow_ID': i, 'Source_ID': sources_d[l], 'Destination_ID': NUM_INP_PORTS+d, 'Size': flow_size};
                flows.append(F);

        cof_sources.append(sources)
        cof_dest_set.append(destination_set_actual);
        cof_dest_src_dict.append(destination_source_dict);
...
coflows = [];
for i in range(NUM_COFLOWS):
    C = {'Coflow_ID': i, 'Flows': [], 'Num_sources':num_sources_list[i], 'Num_destinations':num_destinations_list[i], 'Sources': cof_sources[i], 'Dest_set': cof_dest_set[i], 'Dest_src_dict': cof_dest_src_dict[i]};
    coflows.append(C);
...
# ------Coflow released time generation -------
def PoissonReleaseTimesGenerator(lambda_ = 3):
    poisson_gen = lambda: np.random.poisson(lam=lambda_)
    for C in coflows:
        max_arrival_time = -1;
        count_flows = 0;
        for f in C['Flows']:
        ...
        C['Arrival_Time'] = int(poisson_gen());
...
def ParetoReleaseTimesGenerator(xm_=1,alpha_ = 3.0):  
    for C in coflows:
        u = random.uniform(0.0001, 1.0);  # éviter division par zéro
        pareto_value = xm_ / (u ** (1 / alpha_));
        C['Arrival_Time'] = int(pareto_value);  # toujours entier
        C['Count_Flows'] = len(C['Flows']);

# ------Coflow released time generation Geometric Law-------
def tirage_geometrique(p):
    """Retourne le nombre d'essais avant un succès avec probabilité p."""
    count = 1
...
def GeometricReleaseTimesGenerator(p_ = 0.3):
    t = 0 ; # temps initial
    first = True ; # indicateur pour le premier coflow
    ...
    for C in coflows:
        ...
        if not first:
            interval = tirage_geometrique(p_);  # tirage géométrique
            # print(interval);
            t += interval;
        else:
            first = False;  # ne pas tirer pour le premier coflow

        C['Arrival_Time'] = t;
        C['Count_Flows'] = count_flows;

if (CoflowRelasedTimeMethod=='PS'): 
    lambda_ = int(sys.argv[12]);
    PoissonReleaseTimesGenerator(lambda_);
elif (CoflowRelasedTimeMethod=='P'): 
    alpha_ = float(sys.argv[12]);
    ParetoReleaseTimesGenerator(xm_=1,alpha_ =alpha_);
elif (CoflowRelasedTimeMethod=='G'):
    p_ = float(sys.argv[12]);
    GeometricReleaseTimesGenerator(p_=p_);

lennum = len(str(NUM_SAMPLES - 1))
OUTDIR = sys.argv[9]
if not os.path.exists(OUTDIR):
    os.makedirs(OUTDIR);
#output trace to file
for ns in range(NUM_SAMPLES):
    OUTPUT_FILE = OUTDIR + '/' + FILE  + '-' + str(ns).zfill(lennum) + '.txt'
    outfile = OUTPUT_FILE;
    out_file = open(outfile,'w');
    out_file.write(str(2*NUM_INP_PORTS)+' '+str(NUM_COFLOWS)+'\n');
    ...
    for C in coflows:
        ...
        ## generate the new flow sizes for each trace file
        destination_source_dict = C['Dest_src_dict']
        for d in list(C['Dest_set']):
            num_sources_d = len(destination_source_dict[d]);
            if(num_sources_d==0):
                continue;
            total_data_destination = max(int(np.squeeze(hist_dist_dd.rvs(size=1))),1);
            ...
            for l2 in range(num_sources_d):
                if data_across_sources[l2] <= 0.0 :
                    data_across_sources[l2] = 0.01;
                # flow_size = data_across_sources[l2];  #original code
                flow_size = math.ceil(data_across_sources[l2]);
                F = {'Coflow_ID': i, 'Source_ID': sources_d[l2], 'Destination_ID': NUM_INP_PORTS+d, 'Size': flow_size};
                C['Flows'].append(F);
