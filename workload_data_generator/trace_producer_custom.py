...
ALPHA = (sys.argv[3]); #alpha value for code
# NUM_INP_PORTS = 20;
NUM_INP_PORTS = int(sys.argv[2]);
LOAD_FACTOR = float(sys.argv[4]);
...
if(ALPHA=='FB-UP'):
    OUTDIR = sys.argv[5]
    NUM_FILE = sys.argv[6]
    FILE = 'N'+str(NUM_COFLOWS)+'-a'+str(LOAD_FACTOR)+'-'+'FB-UP-'+str(NUM_FILE)
else:  # not correct
    ALPHA = int(ALPHA)
    INTRA_COFLOW_CONTENTION = float(sys.argv[5]); #c discussed in the code, belongs to [0,1], the traffic is one to ir for c = 0 and all to all for c = 1
    SOURCE_NUM_DIST = sys.argv[6]#'U' for uniform, 'Z' for zipf, 'FB' for trace
    DESTINATION_DATA_DIST = sys.argv[7] #'U' for uniform, 'Z' for zipf, 'N' for normal
    OUTDIR = sys.argv[8]
    NUM_FILE = sys.argv[9]
...
for i in range(NUM_COFLOWS):
    ...
    if(ALPHA=='FB-UP'):
        ...
        sources = np.random.choice(np.arange(0,NUM_INP_PORTS),num_sources,replace=True);
        # sources = np.random.choice(np.arange(0,NUM_INP_PORTS),num_sources,replace=False); # real code
        sources.sort();
        # print(np.arange(0,NUM_INP_PORTS))
        # print(num_destinations)
        destinations = np.random.choice(np.arange(0,NUM_INP_PORTS),num_destinations,replace=True);
        # destinations = np.random.choice(np.arange(0,NUM_INP_PORTS),num_destinations,replace=False); # real code
        destinations.sort();
        ...
        for d in list(destinations_set):
            ...
            # print(hist_dist_dd.rvs(size=1))
            total_data_destination = max(int(scalar(hist_dist_dd.rvs(size=1))),1);
